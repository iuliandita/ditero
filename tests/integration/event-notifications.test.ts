...
const userIds = [OWNER, A, B, OUT] as const;
// The mention handle is the display name, so names must be lexable handles.
...
// Mirrors the /api/zero/mutate handler: collect inside the mutator, enqueue
// only after it resolved. A rejected mutator never reaches enqueue.
...
describe("event rows and the reminder machinery", () => {
	test("the scan's sweep branches select nothing for event-only rows", async () => {
		...
		expect(summary.repeated).toBe(0);
		expect(summary.escalated).toBe(0);
		expect(summary.terminated).toBe(0);
		// No reminder_state row exists to be woken, escalated or terminated, and
		// the event row itself is untouched by the scan.
		expect(
			await db
				.select()
	...
	test("an event enqueued during quiet hours defers to the window end, then delivers", async () => {
		await seedChannel(B, "ntfy");
		// 22:00-07:00 UTC; NOW+13h is 23:00, inside the window.
		await setPref(B, { quietHours: { start: "22:00", end: "07:00" } });
		const at = new Date("2026-07-22T23:00:00Z");
		const collected = await mutate(mutators.task.assign, A, {
			taskId: TASK,
			userId: B,
		});
		...
		expect(rows[0].nextAttemptAt.toISOString()).toBe(
